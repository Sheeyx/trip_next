...
const AddProperty = ({ initialValues, ...props }: any) => {
	...
	async function uploadImages() {
      ...
      // we must provibe token when we use axios request, becuase token will not be provided for axios as we are using graphql api
	...
	const updatePropertyHandler = useCallback(async () => {
		try {
			...
			await sweetMixinSuccessAlert('This property has been updated succeddfully.');
			await router.push({
				pathname: '/mypage',
				query: {
					category: 'myProperties',
